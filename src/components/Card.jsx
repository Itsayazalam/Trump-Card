...
function Card({
  ...
  if (showBack) {
    return (
      <div
        className={`${sizeClasses[size]} bg-blue-600 rounded-lg border-2 border-blue-700 flex items-center justify-center cursor-default shadow-md`}
      >
  ...
  return (
    <div
      ...
      className={`
        ...
        ${
          isSelected
            ? "border-blue-500 bg-blue-50 transform -translate-y-2 -scale-105"
            : "border-gray-300"
        }
      `}
    >
      {!isPlayable && (
        <div className="absolute w-full h-full bg-black/30 -mt-1"></div>
      )}
      {/* Top corner */}
      <div
      ...
      <div
        className={`self-end transform rotate-180 ${
          SUIT_COLORS[card.suit]
        } font-bold leading-none ${!isPlayable && "text-gray-400 opacity-50"}`}
      >
        <div className="text-center">
          <div>{card.value}</div>
          <div className={`text-xs ${!isPlayable && "opacity-10"}`}>
            {SUIT_SYMBOLS[card.suit]}
          </div>
        </div>
      </div>
